...
window.onload = function () {
    ...
    function funTab() {
        ...
        $("#pic").attr("src", arr[index]);
    }

    funTab(); //第一次运行时 调用一次,进行初始化
    slide_icon1.click(prevImg);//当点击前一张按钮时,调用方法 显示前一张图片
    slide_icon2.click(nextImg);//当点击后一张按钮是,调用方法显示后一张图片
 ...
 */
$("#pic").click(function () {
    if($(this).attr("src") == "img/images/pic_2.jpg"){
        window.location.href="specialColumn.html";
    }
});